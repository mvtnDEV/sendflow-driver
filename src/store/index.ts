...
const API =
  process.env.NEXT_PUBLIC_API_URL || "https://sendflow-eta.vercel.app";
  ...
  for (const scan of pending) {
    try {
      const res = await fetch(
        `${API}/api/driver/scan?q=${encodeURIComponent(scan.qrCode)}`,
        {
...
export async function fetchOrderByQr(
  qrCode: string,
  token: string,
): Promise<Order | null> {
  try {
    const res = await fetch(
      `${API}/api/driver/scan?q=${encodeURIComponent(qrCode)}`,
      {
        headers: { Authorization: `Bearer ${token}` },
      },
    );
...
export async function fetchStores(
  token: string,
): Promise<{ id: string; name: string }[] | "SESSION_EXPIRED"> {
...
export interface ScannedOrder {
  ...
  bultos: number;
  status: OrderStatus;
  packKey?: string; // shipping_id de ML si es parte de un pack
  packSize?: number; // cuántas ventas tiene el pack
}

// ── Escaneo con pack: devuelve el pedido y sus hermanas del mismo envío ML ──
export interface ScanResult {
  order: ScannedOrder;
  pack: ScannedOrder[]; // otras ventas del mismo envío (vacío si no es pack)
}

function toScanned(o: any, packKey?: string, packSize?: number): ScannedOrder {
  return {
    id: o.id,
    orderNumber: o.orderNumber,
    customerName: o.customerName,
    addressStreet: o.addressStreet,
    addressComuna: o.addressComuna,
    storeId: o.storeId,
...
export async function scanCode(
  qrCode: string,
  token: string,
): Promise<ScanResult | "NOT_FOUND" | "OFFLINE" | "SESSION_EXPIRED"> {
  try {
    const res = await fetch(
      `${API}/api/driver/scan?q=${encodeURIComponent(qrCode)}`,
      {
    ...
    const data = await res.json();
    if (!data.ok) return "NOT_FOUND";

    const pack: any[] = Array.isArray(data.pack) ? data.pack : [];
    const packKey =
      pack.length > 0
        ? String(data.data?.rawPayload?.shipping?.id ?? "") || undefined
        : undefined;
    const packSize = pack.length > 0 ? pack.length + 1 : undefined;

    return {
      order: toScanned(data.data, packKey, packSize),
      pack: pack.map((p) => toScanned(p, packKey, packSize)),
    };
  } catch {
    addPendingScan(qrCode);
    return "OFFLINE";
...
// ── Actualiza el estado local de varios pedidos (tras recepcionar / salir a ruta) ──
export function setBodegaStatus(ids: string[], status: OrderStatus) {
  const set = new Set(ids);
  const updated = getBodegaPedidos().map((o) =>
    set.has(o.id) ? { ...o, status } : o,
  );
  save(BODEGA_KEY, updated);
  return updated;
}

export function removeBodegaPedidos(ids: string[]) {
  const set = new Set(ids);
  const updated = getBodegaPedidos().filter((o) => !set.has(o.id));
  save(BODEGA_KEY, updated);
  return updated;
}
